...
export default function RegisterPage() {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      pushToast("Unable to register", "error");
    }
  };
  ...
  return (
    <section className="mx-auto flex min-h-[620px] max-w-4xl flex-col justify-center p-6 text-center sm:p-8 reveal">
      ...
      >
        <h1 className="font-serif text-5xl text-zinc-900 dark:text-zinc-100">Create Account</h1>
        <p className="mt-2 text-lg text-zinc-500 dark:text-zinc-400">Join Valley Mountain today</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-2.5 reveal reveal-delay-2">
          <label className="flex h-12 items-center gap-2.5 rounded-lg border border-zinc-300 bg-white px-4 dark:border-zinc-700 dark:bg-zinc-950">
            ...
            <input className="w-full bg-transparent text-base outline-none placeholder:text-zinc-500" placeholder="Name" {...register("name", { required: true })} />
          </label>
          ...
          <label className="flex h-12 items-center gap-2.5 rounded-lg border border-zinc-300 bg-white px-4 dark:border-zinc-700 dark:bg-zinc-950">
            ...
            <input type="password" className="w-full bg-transparent text-base outline-none placeholder:text-zinc-500" placeholder="Password" {...register("password", { required: true, minLength: 6 })} />
          </label>
        ...
        <div className="mt-6 border-t border-zinc-300 pt-6 text-center dark:border-zinc-700">
          ...
          <Link className="mt-1 inline-block text-xl font-semibold text-zinc-900 underline underline-offset-4 dark:text-zinc-100" to="/login">
            Sign In
          </Link>
        </div>
      </div>
