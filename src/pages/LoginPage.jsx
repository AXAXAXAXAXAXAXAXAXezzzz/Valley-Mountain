...
export default function LoginPage() {
  ...
  const { login, googleLogin, user, authReady, needsProfileCompletion } = useAuth();
  ...
  const onSubmit = async (values) => {
    try {
      ...
      pushToast(error?.code ? `Invalid credentials: ${error.code}` : "Invalid credentials", "error");
    }
  };
  ...
  return (
    <section className="mx-auto flex min-h-[620px] max-w-4xl flex-col justify-center p-4 text-center sm:p-8 reveal">
      ...
      >
        <h1 className="font-serif text-4xl text-zinc-900 dark:text-zinc-100 sm:text-5xl">Welcome Back</h1>
        <p className="mt-2 text-base text-zinc-500 dark:text-zinc-400 sm:text-lg">Sign in with username and password</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-2.5 reveal reveal-delay-2">
          <label className="flex h-11 items-center gap-2.5 rounded-lg border border-zinc-300 bg-white px-3 dark:border-zinc-700 dark:bg-zinc-950 sm:h-12 sm:px-4">
            ...
            <input className="w-full bg-transparent text-sm outline-none placeholder:text-zinc-500 sm:text-base" placeholder="Username or name" {...register("identifier", { required: true })} />
          </label>

          <label className="flex h-11 items-center gap-2.5 rounded-lg border border-zinc-300 bg-white px-3 dark:border-zinc-700 dark:bg-zinc-950 sm:h-12 sm:px-4">
            <Lock size={18} className="text-zinc-500" />
            <input type="password" className="w-full bg-transparent text-sm outline-none placeholder:text-zinc-500 sm:text-base" placeholder="Password" {...register("password", { required: true })} />
        ...
        <div className="mt-6 border-t border-zinc-300 pt-6 text-center dark:border-zinc-700">
          ...
          <Link className="mt-1 inline-block text-lg font-semibold text-zinc-900 underline underline-offset-4 dark:text-zinc-100 sm:text-xl" to="/register">
            Create Account
          </Link>
        </div>
      </div>
