...
export default function ProductCard({ product, mode = "grid", onQuickView, compact = false }) {
  ...
  return (
    <article
      className={`reveal relative h-full overflow-hidden rounded-[2rem] border border-zinc-200/90 bg-[#f8f7f4] p-4 shadow-[0_24px_60px_-40px_rgba(0,0,0,0.5)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_72px_-36px_rgba(0,0,0,0.55)] dark:border-zinc-700 dark:bg-zinc-900 ${
    ...
    >
      ...
      <p className={`text-center font-serif text-[11px] uppercase tracking-[0.24em] text-zinc-700 dark:text-zinc-200 ${compact ? "pb-3" : "pb-4"}`}>Valley Mountain</p>

      <Link
        to={`/product/${product._id}`}
        state={{ product }}
        className={`group relative block overflow-hidden rounded-[1.4rem] ${isList ? "h-full min-h-[320px]" : compact ? "h-64" : "h-80"}`}
      >
        ...
        <div className="absolute left-3 top-3">
          <button
            className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-zinc-900 shadow-sm backdrop-blur transition hover:bg-white dark:bg-zinc-900/70 dark:text-zinc-100 dark:hover:bg-zinc-800"
            onClick={(event) => {
              event.preventDefault();
      ...
      <div className={`flex flex-1 flex-col ${isList ? "p-0" : compact ? "px-1 pb-1 pt-4" : "px-1 pb-1 pt-5"}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="mb-2 inline-flex w-fit rounded-full border border-zinc-300 bg-white px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-700 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200">
              {product.category}
            </p>
            <h3 className={`truncate font-serif ${compact ? "text-[1.35rem] leading-tight" : "text-[1.7rem]"} text-zinc-900 dark:text-zinc-100`}>{product.name}</h3>
          </div>
          <p className={`${compact ? "text-xl" : "text-[1.65rem]"} whitespace-nowrap font-semibold [font-family:'Open_Sans',sans-serif] text-zinc-900 dark:text-zinc-100`}>{formatCurrency(product.price)}</p>
        </div>

        {!compact && <p className="mt-2 truncate text-sm text-zinc-500 dark:text-zinc-300">{product.description}</p>}

        <div className={`mt-auto grid grid-cols-2 gap-2.5 ${compact ? "pt-4" : "pt-5"}`}>
          <button
            className={`inline-flex items-center justify-center gap-2 rounded-lg bg-zinc-950 px-3 font-semibold text-white transition hover:bg-black dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200 ${
          ...
          <Link
            to={`/product/${product._id}`}
            state={{ product }}
            className={`inline-flex items-center justify-center rounded-lg border border-zinc-300 bg-transparent px-3 font-semibold text-zinc-800 transition hover:bg-zinc-100 dark:border-zinc-600 dark:text-zinc-100 dark:hover:bg-zinc-800 ${
              compact ? "h-11 text-xs" : "h-12 text-sm"
            }`}
          >
            View Product
